...
<?php
 ...
 foreach ($candoffer_databank as $row){
...
echo    "<td>".anchor_popup('candidates/admin/editCandidate/'.$row->cid, img('http://software.triedge.in/assets/icons/pencil.png'), $atts)."</td>";
 ...
 if($row->stage=='cvsent')
   {
    echo '<td bgcolor="#CCCCFF">'.form_hidden("s_id[]",$row->s_id).form_dropdown('stage[]',$array1, set_value('stage',$row->stage)).'</td>';
   }
   else
   {
 echo    '<td>'.form_hidden("s_id[]",$row->s_id).form_dropdown('stage[]',$array1, set_value('stage',$row->stage)).'</td>';
 }
$atts = array(
              'width'      => '800',
              'height'     => '600',
