...
.data tr, .data td, .data th { 
	...
	</style><link type="text/css" rel="stylesheet" href="<?php echo base_url()?>assets/css/buttons.css" />
			<script type="text/javascript">
    var intTextLnki=1;
  
       var current;//FUNCTION TO ADD TEXT BOX ELEMENT
...
<?php echo $detail['pof_no']; ?> | <b><?php echo $detail['jobtitle']; ?></b> | <?php echo $detail['comp']; ?><br/>
<b>Allocated To </b>- <?php echo $detail['consuls']; ?> | <b>Client Manager</b> - <?php echo $detail['username']; ?> <br/>
<b>Hiring Manager</b> - <?php foreach($hiring_mgr as $key=>$row)
echo $row['name']."--".$row['telephone']."/".$row['mobile']."/".$row['email']."\n";
?>
...
<?php 
	...
	$page = ($this->uri->segment(5))? $this->uri->segment(5) : 0;
		  ?>
	<?php echo form_open('pof/admin/clientChangeCVsent/'.$class.'/'.$pid.'/'.$page); ?>
<?php
 if (count($cvsent_databank)){ 
 echo "<table class='data'>\n";
	echo "<thead>\n";
	echo "<tr>\n";
	echo "<th></th>\n";
	echo "<th></th>\n";
		...
		echo "<th>CV shared with client on</th>\n";
		echo "<th><div align='center'>CV</div></th>\n";
		echo "</tr>\n</thead>\n<tbody>\n";
 
 foreach ($cvsent_databank as $row){
 
echo "<tr valign='top'>\n";
  echo    "<td>".form_checkbox('c_id[]',$row->cid,FALSE)."</td>";
  $atts = array(
              'width'      => '700',
              'height'     => '560',
              'scrollbars' => 'yes',
...
echo    "<td>".anchor_popup('candidates/admin/editCandidate/'.$row->cid, img('http://software.triedge.in/assets/icons/pencil.png'), $atts)."</td>";
 ...
 if($row->stage=='cvsent')
   {
    echo '<td bgcolor="#CCCCFF">'.form_hidden("s_id[]",$row->s_id).form_dropdown('stage[]',$candstagespost, set_value('stage',$row->stage), "style='width:100px;'").'</td>';
   }
   else
   {
 echo    '<td>'.form_hidden("s_id[]",$row->s_id).form_dropdown('stage[]',$candstagespost, set_value('stage',$row->stage),"style='width:100px;'").'</td>';
 }
 echo    "<td><input type='text' name='sharedon[]' class='input-medium datepick' size='8' value='".$row->shared_on."' style='width:90px;'/></td>";
$atts = array(
              'width'      => '800',
              'height'     => '600',
              'scrollbars' => 'yes',
...
<p class="footer">Page rendered in <strong>{elapsed_time}</strong> seconds and Total records found :: <b><?php echo $cvsent; ?></b></p>
  ...
  <tr>
	...
	<table width="1000" id="table2">
      ...
      <tr height="20">
        ...
        <td><div align="center">
          <input name="grade" type="text" size="5" value="<?php echo $detail['grade']; ?>" style="width:80px;"/>
        </div></td>
        <td><div align="center">
        ...
        <td> <div align="center">
		  ...
		  <option value=""></option>
            <option value="cxo" <?php if($detail['level']=='cxo'){ echo 'selected="selected"'; }?>>CXO</option>
            <option value="senior" <?php if($detail['level']=='senior'){ echo 'selected="selected"'; }?>>Senior</option>
            <option value="senior_mid" <?php if($detail['level']=='senior_mid'){ echo 'selected="selected"'; }?>>Senior Mid</option>
            <option value="junior_mid" <?php if($detail['level']=='junior_mid'){ echo 'selected="selected"'; }?>>Junior Mid</option>
            <option value="agent" <?php if($detail['level']=='agent'){ echo 'selected="selected"'; }?>>Agent</option>
            <option value="fresher" <?php if($detail['level']=='fresher'){ echo 'selected="selected"'; }?>>Fresher</option>
          </select>
</div></td>
        <td><div align="center">
...
<table width="847">
  ...
  <tr>
    ...
    <td><select name="transaction_type" style="width:100px;">
      <option value=""></option>
      <option value="contingent" <?php if($detail['transaction_type']=='telephonic'){ echo 'selected="selected"'; }?>>Contingent</option>
  ...
  <tr>
    ...
    <td><?php echo form_dropdown("hiring_mgr",array('0' => '')+$hiringmanager,set_value('hiring_mgr',$detail['hiring_manager']),"id='id', style='width:143px; background-color:#CCCCCC'");?></td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
...
<?php echo form_open('pof/admin/posStrategy/'.$pid); ?>			
  <table>
    <tr>
    <td colspan="13"><h2>Position Search Strategy</h2></td>
  </tr>
  <tr>
    <td width="170" height="26" colspan="4"><h6>Key Selling Propositions </h6></td>
    <td colspan="4"><h6>Key Evaluation Points </h6></td>
    </tr>
  
  
  <tr>
    <td height="26" colspan="4"><div id="content-key" >1
          <input type='text' id='keysell[]' name='keysell[]' style='width:300px;' />
     
    </div></td>
    <td colspan="4"><div id="content-check" >1
          <input type='text' id='keyeva[]' name='keyeva[]' style='width:300px;' />
      
    </div></td>
    </tr>
  <tr>
    <td height="28" colspan="4"><input type="button" name="Input5" value="Add More" onClick="addElementKey();" /></td>
    <td height="28" colspan="4"><input type="button" name="Input6" value="Add More" onClick="addElementCheck();" /></td>
    </tr>
  <tr>
    <td colspan="13">&nbsp;</td>
  </tr>
  ...
  <tr>
    <td colspan="13"><h6>Target Companies </h6></td>
  </tr>
  <tr>
    <td colspan="13"><div id="country" style="width:350px;float:left;">
    Industry : <br/>
    ...
    <script type="text/javascript">
	  	$("#countryid").click(function(){
	    		 if( $('#countryid :selected').length > 0){
        //build an array of selected values
        var countryid= $("#countryid").val();

        $('#countryid :selected').each(function(i, selected) {
            countryid[i] = $(selected).val();
        });

	    			$.ajax({
							type: "POST",
							url : "<?php echo site_url('pof/admin/select_company')?>",
  ...
  <tr>
    <td colspan="13"><h6>Excluded Companies </h6></td>
  </tr>
  <tr>
    <td colspan="13"><div id="country" style="width:350px;float:left;">
    Industry : <br/>
  ...
  <tr>
    <td colspan="13"> <input name="submit" type="submit" class="btn btn-primary" value="Save"></td>
  </tr>
  </table>
  </div>