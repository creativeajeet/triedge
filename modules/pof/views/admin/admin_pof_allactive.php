...
<table width="100%">
  ...
  <tr>
    ...
    <td width="10%"><div align="center"><?php print anchor('pof/admin/allActive','All Active',array('class'=>'icon_pos','id'=>'current'))?></div></td>
...
<?php
 if (count($results)){ 
 echo "<table>\n";
 echo form_open('pof/admin/filterActive/');
	echo "<tr>\n";
	echo "<th></th>\n";
	echo "<th></th>\n";
	echo "<th></th>\n";
...
if(($row->end_date)< date('Y-m-d'))
   {
    echo    "<td bgcolor='#FF0000'>Expired</td>";
   }
  elseif(($row->end_date)==date('Y-m-d'))
   {
    echo    "<td bgcolor='#FFFFCC'>Last day</td>";
   }
  elseif(($row->end_date)=='0000-00-00')
   {
    echo    "<td>No end date</td>";
   }
   else{
     $dateend = strtotime($row->end_date);
 $datetodays = date('Y-m-d');
 $datet = strtotime($datetodays);
 $days_between_talks = ceil(abs($dateend - $datet) / 86400);
 echo    "<td><div align='center'>".$days_between_talks ." days to go</div></td>";
