...
<?php $pofno = $this->uri->segment(4); ?>
<?php $pageno = ($this->uri->segment(5))? $this->uri->segment(5) : 0; ?>
<?php echo form_open('pof/admin/dupRemover/'.$pofno.'/'.$pageno); ?>
...
echo "<tr>\n";
 echo    "<td class='heading'><input class='case' name='cdel_id[]' value='".$row->cid."' type='checkbox'>D</td>";
 echo    "<td class='heading'><input class='case' name='cup_id[]' value='".$row->cid."' type='hidden'><input class='case' value='".$row->id."' type='checkbox'>U</td>";
 echo    "<td class='heading'><input class='case' name='crem_id[]' value='".$row->cid."' type='checkbox'>R</td>";
...
echo    "<td><textarea name='comment[]'>$row->comment</textarea></td>";
 ...
 <?php echo $links; ?><div align="left" style="position:relative">Jump to page no.<input type="text" name="jump" size="6" /><input type="submit" class="btn btn-primary" name="go" value="GO" /></div><div  class="btn" align="right"><i class="icon-step-backward"></i>                
	<?php echo anchor('pof/admin/posPage/'.$pofno.'/'.$pageno, 'Go to Position Page'); ?>
</div><div align="right" style="position:relative; margin-top:-10px; margin-bottom:20px;"><input type="submit" class="btn btn-primary" name="save" value="Done" /></div>
