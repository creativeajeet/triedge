...
echo "<tr valign='top'>\n";
 ...
 ?></td>
	...
	<br/>
	<h2>Client Interview Shorlisted</h2>
	<div id="show">
	<?php 
	...
	$page = ($this->uri->segment(5))? $this->uri->segment(5) : 0;
		  ?>
	<?php echo form_open('pof/admin/clientChange/'.$class.'/'.$pid.'/'.$page); ?>
<?php
 if (count($clientshortlisted_databank)){ 
  echo "<table class='data'>\n";
	echo "<thead>\n";
	echo "<tr>\n";
	echo "<th></th>\n";
 ...
 foreach ($clientshortlisted_databank as $row){
...
echo    "<td>".anchor_popup('candidates/admin/editCandidate/'.$row->cid, img('http://software.triedge.in/assets/icons/pencil.png'), $atts)."</td>";
 ...
 if($row->stage=='cvsent')
   {
    echo '<td bgcolor="#CCCCFF">'.form_hidden("s_id[]",$row->s_id).form_dropdown('stage[]',$candstagespost, set_value('stage',$row->stage)).'</td>';
   }
   else
   {
 echo    '<td>'.form_hidden("s_id[]",$row->s_id).form_dropdown('stage[]',$candstagespost, set_value('stage',$row->stage)).'</td>';
 }
$atts = array(
              'width'      => '800',
              'height'     => '600',
...
<p class="footer">Page rendered in <strong>{elapsed_time}</strong> seconds and Total records found :: <b><?php echo $clientshortlisted; ?></b></p>

</div>
 <?php echo $links; ?>
    <div align="right">
      <input type="submit" class="btn btn-primary" name="change" value="Save Change" />
    </div>
