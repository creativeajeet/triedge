...
<?php if (count($results)){ 
 echo "<table>\n";
echo form_open('synonym/admin/filterind/'.$page);
 echo "<tr>\n";
 echo "<th>".anchor('synonym/admin/index/','<span>All</span>',array('id'=>'all'))."</th>\n";
	echo "<th><input type='text' name='indf' size='50'/><input type='submit' name='Submit' value='Filter' /></th>\n";
	echo "</tr>\n";
		echo form_close();
 echo form_open('synonym/admin/sendtoparent/'.$page);
	...
	echo "<th><div align='left'><input id='selectall' type='checkbox'></div></th>\n";
			echo "<th><div align='left'>Industry Name</div></th>\n";
...
echo "<tr valign='top'>\n";
	echo '<input type="hidden" name="type" value="industry"/>';
  echo    "<td><input class='case' name='synonym[]' value='".$row->industry."' type='checkbox'></td>";
