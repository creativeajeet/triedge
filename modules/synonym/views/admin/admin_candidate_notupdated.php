...
<h2>Company names to be updated</h2>
<div align="right"><div align="right"><span id="updated">Not Updated Company :: <?php echo $total; ?></span><div align="right" style="margin-right:180px; margin-top:-17px;"><span id="notupdated">Total DB :: <?php echo $totaldb; ?></span></div></div></div>
...
<div align="right"><?php 
    ...
    echo anchor_popup('synonym/admin/lastupdated','Last Updated', $atts1); ?> | <?php 
            $atts1 = array(
              'width'      => '900',
              'height'     => '760',
              'scrollbars' => 'yes',
    ...
    echo anchor('synonym/admin/CandManager/'.$page,'Edit Mode'); ?></div>
...
echo "<tr valign='top'>\n";
 ...
 echo    "<td><input class='case' name='cand_id[]' value='".$row->id."' type='hidden'>
 <script>
 $(window).load(function(){
 $('#don".$row->id."').click(function() {
...
<input type='button' id='don".$row->id."' value='X' />
<input class='case' name='cand_id[]' value='".$row->id."' type='hidden'>
 <script>
 $(window).load(function(){
 $('#do".$row->id."').click(function() {
  
    $('#company".$row->id."').val($('#oldcompany".$row->id."').val());


});
});
</script>";
echo "<input type='hidden' size=30 value='".$row->current_company."' id='oldcompany".$row->id."' /><input type='button' id='do".$row->id."' value='Copy' />";

if(!$row->company)
 {
  echo "<input type='text' name='companyname[]' size=30 id='company".$row->id."' /></td>";
 }
 else{
  echo "<input type='text' name='companyname[]' size=30 value='".$row->company."' id='company".$row->id."' /></td>";
 }
