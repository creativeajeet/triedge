...
$(document).ready(function() {
  $("tr#cat'.$row->user_id.'.child").hide();
        $("tr#cat'.$row->user_id.'.header").show();

   $("tr#cat'.$row->user_id.'.header").click(function () { 
      $("tr#cat'.$row->user_id.'.child").each(function() {
 ...
 if(($row->user_id) != $consultant )
				   {
				    echo "<tr id='cat".$row->user_id."' class='header'><th colspan='17' id='employee'>".anchor_popup('candidates/admin/editCandidate/'.$row->client_id, img('http://software.triedge.in/assets/icons/pencil.png'), $atts)." ".$row->username."<div align='left' style='margin-top:-10px; margin-bottom:-30px; margin-left:1040px;'><b>Client Manager :</b></div><div align='right' style='margin-top:-10px; margin-bottom:10px;'>".anchor_popup('companies/admin/addScribble/'.$compid.'/'.$row->client_id, img($scribble_image), $atts)."</div></th></tr>";
...
if(($row->talked_on)==date('Y-m-d'))
  {
   echo "<tr valign='top' bgcolor='#FFFFCC' id='cat".$row->user_id."' class='child'>\n";
  }
  else{
 if(($row->manager)==19)
 {
echo "<tr valign='top' bgcolor='#FFCCCC' id='cat".$row->user_id."' class='child'>\n";
}
elseif(($row->manager)==37)
 {
echo "<tr valign='top' bgcolor='#66FF99' id='cat".$row->user_id."' class='child'>\n";
}
else
{
echo "<tr valign='top' bgcolor='#eee' id='cat".$row->user_id."' class='child'>\n";
