...
$date = $this->uri->segment(4);
		...
		$day1 = date('D',strtotime($date . '-1 days'));
		$day2 = date('D',strtotime($date . '-2 days'));
		$day3 = date('D',strtotime($date . '-3 days'));
		$day4 = date('D',strtotime($date . '-4 days'));
		$day5 = date('D',strtotime($date . '-5 days'));
		$day6 = date('D',strtotime($date . '-6 days'));
?>
<div align="right"><?php echo anchor('usage/admin/filterposDelivery/'.$date, 'All Allocated'); ?> | <?php echo anchor('usage/admin/filterposDeliveryActive/'.$date, 'Active Positions'); ?></div>
...
echo    "<td>".anchor('pof/admin/posPage/'.$row->posi, img('http://software.triedge.in/assets/icons/pencil.png'))."</td>";
 ...
 echo "<td><div align='center'>".anchor('usage/admin/posdelcvsent/'.$date.'/'.$row->c_id,'<span id="red">'.$row->count1.'</span>',array('target'=>'_blank'))."</div></td>\n";
     ...
     echo "<td><div align='center'>".anchor('usage/admin/posdelcvsent/'.$date.'/'.$row->c_id,$row->count1)."</div></td>";
   }
 ...
 echo "<td><div align='center'>".anchor('usage/admin/posdelcvsent/'.$date1.'/'.$row->c_id,'<span id="red">'.$row->count2.'</span>',array('target'=>'_blank'))."</div></td>\n";
     ...
     echo "<td><div align='center'>".anchor('usage/admin/posdelcvsent/'.$date1.'/'.$row->c_id,$row->count2)."</div></td>";
   }
 ...
 echo "<td><div align='center'>".anchor('usage/admin/posdelcvsent/'.$date2.'/'.$row->c_id,'<span id="red">'.$row->count3.'</span>',array('target'=>'_blank'))."</div></td>\n";
     ...
     echo "<td><div align='center'>".anchor('usage/admin/posdelcvsent/'.$date2.'/'.$row->c_id,$row->count3)."</div></td>";
   }
 ...
 echo "<td><div align='center'>".anchor('usage/admin/posdelcvsent/'.$date3.'/'.$row->c_id,'<span id="red">'.$row->count4.'</span>',array('target'=>'_blank'))."</div></td>\n";
     ...
     echo "<td><div align='center'>".anchor('usage/admin/posdelcvsent/'.$date3.'/'.$row->c_id,$row->count4)."</div></td>";
   }
 ...
 echo "<td><div align='center'>".anchor('usage/admin/posdelcvsent/'.$date4.'/'.$row->c_id,'<span id="red">'.$row->count5.'</span>',array('target'=>'_blank'))."</div></td>\n";
     ...
     echo "<td><div align='center'>".anchor('usage/admin/posdelcvsent/'.$date4.'/'.$row->c_id,$row->count5)."</div></td>";
   }
 ...
 echo "<td><div align='center'>".anchor('usage/admin/posdelcvsent/'.$date5.'/'.$row->c_id,'<span id="red">'.$row->count6.'</span>',array('target'=>'_blank'))."</div></td>\n";
     ...
     echo "<td><div align='center'>".anchor('usage/admin/posdelcvsent/'.$date5.'/'.$row->c_id,$row->count6)."</div></td>";
   }
 ...
 echo "<td><div align='center'>".anchor('usage/admin/posdelcvsent/'.$date6.'/'.$row->c_id,'<span id="red">'.$row->count7.'</span>',array('target'=>'_blank'))."</div></td>\n";
     ...
     echo "<td><div align='center'>".anchor('usage/admin/posdelcvsent/'.$date6.'/'.$row->c_id,$row->count7)."</div></td>";
   }
	 
		   $totalcvsent = (($row->totalcvsent1)+($row->totalcvsent2)+($row->totalcvsent3)+($row->totalcvsent4));
  echo "<td><div align='center'><b>".anchor('pof/admin/cvSent/'.$row->pofid,$totalcvsent,array('target'=>'_blank'))."</b></div></td>";
  $atts = array(
              'width'      => '800',
              'height'     => '450',
              'scrollbars' => 'yes',
