...
<?php echo form_close(); ?>
	<div id="tab-4">
			<h2>Position Date of Receipt Wise</h2>
...
$(document).ready(function() {
  $("tr#cat'.$row->pof_id.'.child").hide();
        $("tr#cat'.$row->pof_id.'.header").show();

   $("tr#cat'.$row->pof_id.'.header").click(function () { 
      $("tr#cat'.$row->pof_id.'.child").each(function() {
 ...
 if(($row->pof_id) != $dor )
				   {
				    echo "<tr id='cat".$row->pof_id."' class='header'><th colspan='17' id='employee'>".anchor_popup('candidates/admin/editCandidate/'.$row->pof_id, img('http://software.triedge.in/assets/icons/pencil.png'), $atts)." ".$row->pos_taken_on."<div align='left' style='margin-top:-10px; margin-bottom:-30px; margin-left:1040px;'><b>Client Manager :</b>".$row->username."</div><div align='right' style='margin-top:-10px; margin-bottom:10px;'>".anchor_popup('companies/admin/addScribble/'.$compid.'/'.$row->pof_id, img($scribble_image), $atts)."</div></th></tr>";
...
if(($row->talked_on)==date('Y-m-d'))
  {
   echo "<tr valign='top' bgcolor='#FFFFCC' id='cat".$row->pof_id."' class='child'>\n";
  }
  else{
 if(($row->manager)==19)
 {
echo "<tr valign='top' bgcolor='#FFCCCC' id='cat".$row->pof_id."' class='child'>\n";
}
elseif(($row->manager)==37)
 {
echo "<tr valign='top' bgcolor='#66FF99' id='cat".$row->pof_id."' class='child'>\n";
}
else
{
echo "<tr valign='top' bgcolor='#eee' id='cat".$row->pof_id."' class='child'>\n";
